...
<table>
	...
	<table border="1" cellpadding="4" cellspacing="0" style="width: 100%">
		...
		<?php
		$bu = $this->db->query("SELECT * FROM tb_bidang_usaha where token_permohonan ='$p->token_permohonan'")->result();
		$no=0;
		foreach($bu as $row){
			$no++;
			$k = $this->db->query("
		...
		<tr>
			...
			<td valign="top"><?php echo $row->npwp ?></td>
			<td valign="top"><?php echo $row->nama_bu ?></td>
			<td valign="top">
				<table>
					<?php foreach($k as $row){ ?>
					<tr><td><?php echo $row->kode_sub_klasifikasi ?></td></tr>
					<?php } ?>
				</table>
			</td>
			<td valign="top">
				<table>
					<?php foreach($k as $row){ ?>
					<tr><td><?php echo $row->kode_kualifikasi ?></td></tr>
					<?php } ?>
				</table>
			</td>
			<td valign="top">
